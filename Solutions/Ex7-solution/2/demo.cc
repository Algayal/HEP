...
void myWidth() {
    int error;
    int excl;
    std::complex<double> couplings[ncouplings];
    std::complex<double> couplingsms[ncouplingsms];
    double gammas[ngammas];
    double gammasms[ngammasms];

    
    std::ofstream outfile("width-feynhiggs.txt");

      //Redefine constants used in FHSetPara function
     cRealType MT = 173.2;
      cRealType TB = 5;
      cRealType MHp = -1;
    ...
    for (double mass = 80.0; mass <= 140; mass ++){ 
      // updating the mass then calling FHSetPara function    
      cRealType MA0=mass;
        ...
        M_1, M_2, M_3,
        Qtau, Qt, Qb);
      // Now the Higgs mass is updated
      // Caluculating the width and ouputting into a file     
      FHCouplings(&error, couplings, couplingsms, gammas, gammasms, excl);
      double width = GammaSMTot(3);  
      outfile << mass << "\t" << width << "\n";
    }
    outfile.close();
